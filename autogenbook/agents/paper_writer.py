...
class PaperSectionWriterAgent:
    def __init__(self, llm: OpenRouterLLM) -> None:
        self.name = "paper_section_writer"
        self.llm = llm

    def run(self, input: Dict[str, Any], context: AgentContext) -> str:
        llm = context.llm or self.llm
        if llm is None:
            raise RuntimeError("PaperSectionWriterAgent requires an LLM in AgentContext or constructor.")

        prompt_args = dict(input)
        prompt_args.setdefault("retrieved_context", "(none)")
        prompt_args.setdefault("section_draft", "")
        system_prompt = render(get_prompt("paper_section_writer_system"), **prompt_args)
        user_prompt = render(get_prompt("paper_section_writer_user"), **prompt_args)
