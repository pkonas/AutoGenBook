...
@register_agent("book_section_revision")
class BookSectionRevisionAgent:
    def __init__(self, llm: OpenRouterLLM) -> None:
        self.name = "book_section_revision"
        self.llm = llm

    def run(self, input: Dict[str, Any], context: AgentContext) -> str:
        llm = context.llm or self.llm
        if llm is None:
            raise RuntimeError("BookSectionRevisionAgent requires an LLM in AgentContext or constructor.")
