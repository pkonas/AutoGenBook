...
def main() -> int:
    ...
    metadata["components"]["pandoc"]["version"] = capture([str(pandoc_target), "--version"]).splitlines()[0]
    ...
    metadata["components"]["ffmpeg"]["version"] = capture([str(ffmpeg_target), "-version"]).splitlines()[0]

    tinytex_target = tools / "tinytex"
    if tinytex_target.exists():
        shutil.rmtree(tinytex_target)
    shutil.copytree(tinytex_root, tinytex_target, symlinks=True)
    ...
    metadata["components"]["piper_voice_cs_CZ_jirka_medium"] = {
        ...
        "license": "MIT (repository metadata/model card)",
        "model": model,
        "config": config,
        "model_card": card,
    }
    ...
    notices.write_text(
        ...
        "- FFmpeg from `imageio-ffmpeg` — see the binary build configuration and FFmpeg license output.\n"
        "- TinyTeX / TeX Live packages — individual TeX Live package licenses apply.\n"
        "- Piper runtime (`piper-tts`) — MIT.\n"
        "- Czech Piper voice `cs_CZ-jirka-medium` — MIT according to the source repository metadata.\n"
